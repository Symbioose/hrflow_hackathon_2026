...
export default function LoginPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    if (data.user) {
      await supabase.from("user_profiles").insert({ id: data.user.id, name, company });
    }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: "radial-gradient(ellipse at center, #0d1a2e 0%, #080c14 70%)",
        }}
      >
        {/* Grid lines */}
        <div
        ...
        <div className="flex flex-col gap-8 max-w-sm mx-auto w-full">
          ...
          {mode === "login" ? (
            <form onSubmit={handleLogin} className="flex flex-col gap-4">
              ...
              {error && (
                <p className="text-xs px-4 py-3 rounded-xl" style={{ background: "rgba(239,68,68,0.08)", color: "#f87171", border: "1px solid rgba(239,68,68,0.15)" }}>
                  {error}
                </p>
              )}
            ...
            <form onSubmit={handleSignup} className="flex flex-col gap-4">
              <Input
                ...
                hint="Demandez votre token à l'équipe Claw4HR"
              />
